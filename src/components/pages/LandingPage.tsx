...
export function LandingPage({
  ...
  const { data: featuredRegions, isLoading: isLoadingRegions } = useFeaturedRegions();
  const { data: featuredTours, isLoading: isLoadingTours } = useFeaturedTours(3);
  const { data: guides } = useAllGuides();
  const { getImagesByContext, getImageUrl } = useWebsiteImages();
    ...
    <div className="min-h-screen bg-cream">
        ...
        <div className="lg:w-1/2 bg-charcoal text-white flex items-center justify-center p-8 lg:p-16">
          <div className="max-w-2xl">
            ...
            <div className="grid grid-cols-3 gap-6 mb-12 pb-12 border-b border-white/20">
              ...
              <div>
                <div className="text-3xl lg:text-4xl text-burgundy-light mb-2" style={{ fontFamily: 'Playfair Display, serif' }}>{totalGuides}</div>
                <div className="text-sm text-cream/70 uppercase tracking-wider">Certified</div>
              </div>
              <div>
                <div className="text-3xl lg:text-4xl text-burgundy-light mb-2" style={{ fontFamily: 'Playfair Display, serif' }}>{totalRoutes}+</div>
              ...
              return (
                <Card key={tour.id} className="overflow-hidden border-burgundy/10 hover:shadow-2xl transition-all duration-300 cursor-pointer" onClick={() => navigate(`/tour/${tour.slug}`)}>
                  <div className="relative h-64">
                    <SmartImage
                      category="tour"
                      usageContext={`tour-${tour.region_country}-${tour.region_subregion}`}
                      tags={[
                        `location:${tour.region_country.toLowerCase()}-${(tour.region_region || tour.region_subregion).toLowerCase()}`,
                        tour.region_country.toLowerCase(),
                        'hiking',
                        'mountain'
                      ]}
                      className="w-full h-full object-cover"
              ...
              {topGuides.map((guide) => {
                ...
                return (
                  <Card 
                    ...
                    onClick={() => navigate(`/guide/${guideSlug}`)}
                  >
                    <div className="relative h-80">
                      {guide.profile_image_url ? (
                        ...
                        <SmartImage
                          category="guide"
                          usageContext="profile_portrait"
                          tags={['guide', 'portrait', 'professional']}
                          className="w-full h-full object-cover"
                          alt={guide.display_name}
                        />
                      )}
      ...
      <section className="py-24 bg-cream">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="mt-16 text-center">
            ...
            <p className="text-charcoal mt-2" style={{ fontFamily: 'Playfair Display, serif' }}>
      ...
      <section className="relative min-h-screen flex items-center justify-center">
        <div className="absolute inset-0">
          <SmartImage
            category="landscape"
            usageContext="trail_junction"
            tags={['trail', 'path', 'junction', 'mountain']}
            className="w-full h-full object-cover"
            alt="Mountain trail junction signpost"
          />
      ...
      <footer className="bg-charcoal text-white py-16">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="text-center mb-12 pb-12 border-b border-white/10">
            <h3 
              className="text-3xl mb-4 text-burgundy-light"
              style={{ fontFamily: 'Playfair Display, serif' }}
            >
              MADETOHIKE
            </h3>
