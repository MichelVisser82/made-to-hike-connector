...
export function AutomatedResponsesSettings() {
  ...
  const fetchAutomatedMessages = async () => {
    ...
    const {
      data,
      error
    } = await supabase.from('automated_messages').select('*').eq('guide_id', user.id).order('created_at', {
      ascending: false
    });
    if (error) {
  ...
  const handleCreate = async () => {
    ...
    const {
      error
    } = await supabase.from('automated_messages').insert({
      guide_id: user?.id,
  ...
  const handleToggle = async (id: string, isActive: boolean) => {
    const {
      error
    } = await supabase.from('automated_messages').update({
      is_active: !isActive
    }).eq('id', id);
    if (error) {
      toast({
  ...
  return <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle>Setup Responses</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              {quickTemplates.map((template, idx) => <Button key={idx} variant="outline" size="sm" className="justify-start h-auto py-2 px-3 text-left border-burgundy/20 hover:bg-burgundy/5" onClick={() => {
          ...
          <div className="space-y-4">
            <div>
              ...
              <Textarea value={newMessage} onChange={e => setNewMessage(e.target.value)} placeholder="Hi {{hiker_name}}! Thanks for your interest in {{tour_title}}. I'll get back to you within 24 hours." rows={4} />
            </div>

            <div>
              <Label>Delay (minutes)</Label>
              <Input type="number" value={delayMinutes} onChange={e => setDelayMinutes(parseInt(e.target.value) || 0)} placeholder="0" min="0" />
            </div>
      ...
      {messages.length > 0 && <Card>
          ...
          <CardContent>
            <div className="space-y-4">
              {messages.map(msg => <div key={msg.id} className="flex items-start justify-between p-4 border rounded-lg border-burgundy/10">
                  ...
                  <div className="flex items-center gap-2">
                    <Switch checked={msg.is_active} onCheckedChange={() => handleToggle(msg.id, msg.is_active)} className="data-[state=checked]:bg-burgundy" />
                    <Button variant="ghost" size="icon" onClick={() => handleDelete(msg.id)} className="text-burgundy hover:bg-burgundy/5">
