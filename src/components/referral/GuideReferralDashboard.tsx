...
export const GuideReferralDashboard = ({ userId }: { userId: string }) => {
  ...
  const handleShareWhatsApp = (type: "hiker" | "guide") => {
    ...
    const message = type === "hiker"
      ? `Check out Made to Hike - certified guides across the Alps! ${link}`
      : `Hey fellow guide! Join Made to Hike where you keep 95% of your earnings. No marketplace fees. ${link}`;
    
    const whatsappUrl = `https://wa.me/?text=${encodeURIComponent(message)}`;
    window.open(whatsappUrl, '_blank');
  };

  const handleShareLinkedIn = (type: "hiker" | "guide") => {
    const link = type === "hiker" ? links?.hikerLink : links?.guideLink;
    if (!link) return;
  ...
  const handleShareTwitter = (type: "hiker" | "guide") => {
    const link = type === "hiker" ? links?.hikerLink : links?.guideLink;
    if (!link) return;
    
    const text = type === "hiker"
      ? "Explore the Alps with certified guides on Made to Hike"
      : "Join Made to Hike - the guide-first platform where you keep 95% of your earnings";
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-charcoal/70">
            Invite hikers and fellow guides to earn credits. €25 per hiker, €50 per guide.
          </p>
        </div>
            ...
            <div className="space-y-6">
              <div>
                ...
                <p className="text-sm text-charcoal/70 mb-3">
                  Invite hikers (€25 each) and fellow guides (€50 each). Track their progress through verification, first tour publication, and completion.
                </p>
              </div>
              <div>
