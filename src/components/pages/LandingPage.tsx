          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            return <Card key={tour.id} className="overflow-hidden border-burgundy/10 hover:shadow-2xl transition-all duration-300 cursor-pointer" onClick={() => navigate(`/tours/${tour.slug}`)}>
                  <div className="relative h-64">
                    {tour.hero_image ? <img src={tour.hero_image} className="w-full h-full object-cover" alt={`${tour.title} in ${tourRegion}`} /> : <SmartImage category="tour" usageContext={`tour-${tour.region_country}-${tour.region_subregion}`} tags={[`location:${tour.region_country.toLowerCase().replace(/\s+/g, '-')}-${tour.region_subregion.toLowerCase().replace(/\s+/g, '-')}`, 'hiking', 'mountain']} className="w-full h-full object-cover" alt={`${tour.title} in ${tourRegion}`} />}
          ...
          <div className="relative">
            ...
            <div className="relative overflow-hidden">
              ...
              position: index === 0 ? 'relative' : 'absolute'
                    ...
                    {region.hero_image ? <img src={region.hero_image} className="w-full h-full object-cover" alt={`${region.tour_title || region.country + ' - ' + (region.region || region.subregion)}`} /> : <SmartImage category="landscape" usageContext="landing" tags={[`location:${region.country.toLowerCase().replace(/\s+/g, '-')}-${region.subregion.toLowerCase().replace(/\s+/g, '-')}`, 'mountain', 'alpine']} className="w-full h-full object-cover" alt={`${region.country} - ${region.region || region.subregion} mountain landscape`} />}
